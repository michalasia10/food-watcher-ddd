...
async def test_product_controller_create_product(api_client, endpoint_enum, user_token):
    ...
    # when
    response = await api_client.post(endpoint_enum.PRODUCTS.value, json_data=product_input.dict())
...
async def test_consumption_controller_create_consumption(
    ...
    response = await api_client.post(
        endpoint_enum.CONSUMPTION,
        json_data=daily_product_input.model_dump_json()
    )
