...
def test_command_create_recipe(
    recipe_repo: RecipeRepository, recipe_product_repo, product_repo, product_fix
):
    ...
    recipe_dto = RecipeInputDto(
        ...
        link="test",
        products=[
            RecipeProductInputDto(product_id=product_fix.id, weight_in_grams=100)
        ],
    )
    ...
    assert str(recipe_product.product_id) == str(
        expected_product.product_id
    ), "Recipe product not match"
