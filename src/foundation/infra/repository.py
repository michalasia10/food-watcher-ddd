...
#                 seen[obj][relationship.key] = [
#                     to_dict_recursive(child, seen) for child in related_obj
#                 ]
#             else:
...
#
#     def entity_to_model(
#         self, entity: [Entity], model: Model | None = None, update=False
#     ) -> [Base]:
#         entity_dict = {
#             key: value
#             for key, value in vars(entity).copy().items()
#             if not key.startswith("_")
#         }
#         for key, value in entity_dict.items():
#             if hasattr(getattr(entity, key), "Meta") and hasattr(
#                 getattr(entity, key).Meta, "orm_model"
#             ):
#                 child = self.entity_to_model(
#                     getattr(entity, key),
#                     getattr(entity, key).Meta.orm_model,
#                     update=update,
#                 )
...
#                 children = [
#                     self.entity_to_model(item, update=update)
#                     for item in getattr(entity, key)
#                 ]
...
#             relation = [
#                 getattr(self.model, child) for child in self.model.Meta.children
#             ]
#         else:
#             relation = []
#
...
#             if isinstance(value, list):
#                 values_as_model = [
#                     self.entity_to_model(item, update=True)
#                     for item in value
...
#             print(
#                 f"Record with id {entity.id} updated for key {key} with value {value}"
#             )
#         if raw:
#             return record
#         return entity
...
#         self, field: str, value: Any, raw=False
#     ) -> tuple[Any] | None | Any:
#         if hasattr(self.model, "Meta") and hasattr(self.model.Meta, "children"):
#             relation = [
#                 getattr(self.model, child) for child in self.model.Meta.children
