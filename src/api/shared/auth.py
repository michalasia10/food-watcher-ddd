...
class JWTAUthBearer:
    ...
    async def __call__(self, token: HTTPAuthorizationCredentials = Depends(http_bearer)):
        if token:
...
@inject
async def bearer_auth(user_repository: UserRepository = dependency(Container.user_repository),
