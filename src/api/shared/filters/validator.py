...
class QueryValidator:
    ...
    def _parse_field_value(self, filter_key: str, filter_value: str) -> str | dict:
        if '.' in filter_key:
            ...
            return {field_name: self._parse_field_value('.'.join(nested_fields), filter_value)}
    ...
    def _get_field_by_alias(self, filter_key: str):
        field = next((f for f in self.filter_model.__fields__.values() if f.alias == filter_key), None)
        if not field:
            raise HTTPException(status_code=400, detail=f"Invalid field '{filter_key}' in filter")
        return field
