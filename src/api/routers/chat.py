...
class ChannelsViewSet(BaseModelView[ChannelInputDto, ChannelOutputDto]):
    ...
    @inject
    def __init__(self, query_service=dependency(Container.chanel_query),
                 command_service=dependency(Container.chanel_command)):
        super(ChannelsViewSet, self).__init__(query_service=query_service,
...
async def send_message(channel_id: ChanelId,
    ...
    publisher = NotificationPublisherFacade(channel_id, message, rabitmq_url, message_repository, loop, user)
    try:
...
async def chat(websocket: WebSocket,
               ...
               composite_message_repository: MessageCompositeRepository = dependency(
                   Container.composite_message_repository)):
    facade = WebSocketFacade(websocket, channel_id, conn_manager, loop, rabitmq_url, composite_message_repository)
    await facade.run()
