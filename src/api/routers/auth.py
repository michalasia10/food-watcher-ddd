...
from modules.auth.app.usecases.dtos import UserAuthInputDto, TokenOutputDto, UserCreateInputDto, UserOutputDto
...
from src.modules.auth.app.usecases.query.user import UserQuery
...
@router.get('/users/')
@inject
def users(user: HTTPBearer = Depends(bearer_auth),
          query: UserQuery = dependency(Container.user_query),
          ) -> list[UserOutputDto]:
