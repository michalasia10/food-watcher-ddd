...
from src.modules.recipes.app.repository.recipe import (
    RecipeRepository,
...
from src.modules.recipes.domain.entities import (
    Recipe as RecipeE,
    ProductRecipe as ProductRecipeE,
)
from src.modules.recipes.domain.exceptions import (
    RecipeNotFound,
    RecipeAlreadyExists,
...
class RecipeCommand(RecipeCommandBase):
    ...
    def _create_entities_with_macros(
        self, recipe: RecipeInputDto
    ) -> tuple[RecipeE, list[tuple[Any, ProductRecipeE]]]:
        recipe_products_e: list[tuple[Any, ProductRecipeE]] = []
        recipe_e = RecipeE(
            name=recipe.name, description=recipe.description, link=recipe.link
        )
        product: RecipeProductInputDto
        for product in recipe.products:
            base_product = self._product_repository.get_by_id(
                product.product_id, raw=True
            )
    ...
    def _create_recipe_in_db(
        self, recipe_e: RecipeE, recipe_products_e: list[tuple[Any, ProductRecipeE]]
    ):
        ...
        for recipe_product_e in recipe_products_e:
            ...
            product_recipe = self._recipe_product_repository.get_by_field_values(
                raw=True, **values
            )
...
class RecipeProductCommand(RecipeProductCommandBase):
    ...
    def update(
        self, id: RecipeID, recipe_product: RecipeProductInputDto
    ) -> RecipeProductOutputDto:
        if not self._repository.exists("id", id):
            raise RecipeProductNotFound("Recipe not found.")

        recipe_product_record = self._repository.update(
            id, ProductRecipeE(weight_in_grams=recipe_product.weight_in_grams)
        )
