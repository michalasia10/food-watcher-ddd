...
@final
class SqlChannelRepository(Repository, ChannelRepositoryBase):
    model = Channel
...
@final
class SqlMessageRepository(Repository, MessageRepositoryBase):
    model = Message
    ...
    def get_all_by_channel(self, channel_id: ChanelId) -> list[MessageE]:
        data = self.session.query(self.model).filter_by(channel_id=str(channel_id)).all()
        return [self.data_to_entity(dat, MessageE) for dat in data]


@final
class SqlMessageCompositeRepository(MessageCompositeRepositoryBase):

    def __init__(self, channel_repository: ChannelRepositoryBase, message_repository: MessageRepositoryBase):
