...
class ProductQuery(ProductQueryBase):
    def __init__(self, repository: ProductRepository, recipe_repo):
        self._repository = repository
        self._recipe_repo = recipe_repo

    def _map_to_dto(self, product: Product) -> dict:
        product_dict = product.to_dict()
        product_dict["recipes"] = [
            self._recipe_repo.get_by_id(product.recipe_id)
            for product in product.product_for_recipes
        ]
    ...
    def get_all(self, skip: int = 0, limit: int = 100) -> list[ProductOutputDto]:
        return [
            ProductOutputDto(**self._map_to_dto(product))
            for product in self._repository.get_all_pagination(skip, limit)
        ]
    ...
    def get_by_name(self, name: str, skip=0, limit=10) -> list[ProductOutputDto]:
        return [
            ProductOutputDto(**self._map_to_dto(product))
            for product in self._repository.get_all_by_name(
...
class UserDayQuery(UserDayQueryBase):
    ...
    def get_all_user_days(
        ...
        return [
            DailyUserConsumptionOutputDto(**day.to_dict())
            for day in self._repository.get_all_pagination(skip, limit, user_id=user_id)
            if day
        ]
    ...
    def get_day_by_datetime(
        ...
        day = self._repository.get_by_field_values(
            raw=False, date=datetime, user_id=user_id
        )
