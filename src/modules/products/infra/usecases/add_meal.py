...
class AddMealI(AddMeal):
    ...
    def execute(self, daily_product_input: DailyUserProductInputDto) -> None:
        product = self._product_repository.get_by_id(
            daily_product_input.product_id, raw=True
        )
        if product is None:
            raise ProductNotFound(
                f"Product with id {daily_product_input.product_id} not found"
            )

        day: DailyUserConsumption = (
            self._daily_user_consumption_repository.get_by_field_values(
                user_id=daily_product_input.user_id,
                date=self._convert_string_to_date(daily_product_input.date),
            )
        )
        if day is None:
            record = DailyUserConsumption(
                user_id=daily_product_input.user_id,
                date=self._convert_string_to_date(daily_product_input.date),
            )
            day: DailyUserConsumption = self._daily_user_consumption_repository.create(
                record
            )

        daily_product_dto = DailyUserProduct(
            day_id=day.id,
        ...
        daily_product = self._daily_product_repository.create(
            daily_product_dto, raw=True
        )
